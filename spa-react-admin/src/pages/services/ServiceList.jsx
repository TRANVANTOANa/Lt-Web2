...
import ServiceDelete from './ServiceDelete.jsx';
import ViewDetailModal from '../../components/ViewDetailModal.jsx';

export default function ServiceList({ config }) {
  ...
  return (
    <div>
      ...
      <div className="table-card"><table className="data-table"><thead><tr>{config.columns.map(c => <th key={c[0]}>{c[1]}</th>)}<th>Hành động</th></tr></thead><tbody>{filtered.map(row => <tr key={row.id}>{config.columns.map(c => (
                  <td key={c[0]}>
                    {c[2] === 'status' ? (
                      ...
                      row[c[0]] ? (
                        <img
                          src={row[c[0]].startsWith('http') ? row[c[0]] : `${imageHost}${row[c[0]]}`}
                          alt="Service"
                          style={{ width: '60px', height: '40px', objectFit: 'cover', borderRadius: '6px', border: '1px solid var(--line)' }}
                        />
                      ) : (
      ...
      {modal && <ServiceForm fields={config.fields} initial={editing} onClose={() => setModal(false)} onSubmit={save} lookups={lookups} />}
      {deleting && <ServiceDelete item={deleting} onConfirm={handleDelete} onCancel={() => setDeleting(null)} />}
      {viewing && (
        <ViewDetailModal
