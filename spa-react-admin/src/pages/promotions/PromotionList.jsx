...
import PromotionDelete from './PromotionDelete.jsx';
...
export default function PromotionList({ config }) {
  ...
  return (
    <div>
      ...
      {modal && <PromotionForm fields={config.fields} initial={editing} onClose={() => setModal(false)} onSubmit={save} lookups={lookups} />}
      {deleting && <PromotionDelete item={deleting} onConfirm={handleDelete} onCancel={() => setDeleting(null)} />}
    </div>
  );
}
