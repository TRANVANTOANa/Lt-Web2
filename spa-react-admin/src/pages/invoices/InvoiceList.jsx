...
export default function InvoiceList({ config }) {
  ...
  return (
    <div>
      ...
      {modal && <InvoiceForm fields={config.fields} initial={editing} onClose={() => setModal(false)} onSubmit={save} lookups={lookups} />}
      {deleting && <InvoiceDelete item={deleting} onConfirm={handleDelete} onCancel={() => setDeleting(null)} />}
    </div>
  );
}
